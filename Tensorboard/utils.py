...
#Essa pasta contem algumas funcoes uteis, criados com o proposito de visualizar imagens
#Eu roubei elas do tensorflow guide

def plot_to_image(figure):
	"""Convert o matplotlib especificado da imagen para png e retorna o. 
	A imagen disponibilizada depois disso e inacessivel."""

    # Salva o plot em png na memoria
	buf = io.BytesIO()
	plt.savefig(buf, format="png")

    # Fechar a imagen previne que ela seja disponibilizada, diretamente dentro
    # do notebook.
	plt.close(figure)
	buf.seek(0)
...
def image_grid(data,labels,class_names):
	#A data precisa estar nesse formato (BATCH_SIZE,H,W,C)
	assert data.ndim == 4

	figure=plt.figure(figsize=(10,10))
	num_images = data.shape[0]
	size=int(np.ceil(np.sqrt(num_images)))
	#O tamnho das imagens na grid, vai ser o tamnho da imagen raizada. E ele vai ser limitado por essa integer
	...
	for i in range(data.shape[0]):
		...
		#Se greyscale
		if data.shape[3] == 1:
			#Como voce pode ver o shape tem que ser igual a 1
